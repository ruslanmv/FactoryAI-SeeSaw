...
def extract_code(llm_output: str) -> str:
    ...
    """
    # Match triple backtick blocks with optional language tags (e.g., ```python)
    code_blocks = re.findall(r"```(?:\w+\n)?(.*?)```", llm_output, re.DOTALL)
    # Strip whitespace and join multiple code blocks with two newlines
    return "\n\n".join(block.strip() for block in code_blocks if block.strip()) or "No valid code found."



async def validator_function(main_code: str, dependency_code: str) -> (bool, str):
    """
...
async def see_saw_mechanism(project_tree: list):
    ...
    "Do not include comments or explanations. Only return the raw code content"
            ...
            for dep in dependencies:
                ...
                is_valid, updated_main_code = await validator_function(main_code, dep_code)
                if not is_valid:
